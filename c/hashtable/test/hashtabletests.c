...
void setup(void) { ht_init(&t, CAPACITY); }

void teardown(void) { ht_free(&t); }

Test(hashtabletests, put_not_existing, .init = setup, .fini = teardown) {
  cr_expect(ht_put(t, "Name", "Gandalf"),
...
Test(hashtabletests, get_not_existing, .init = setup, .fini = teardown) {
  ...
  cr_expect_null(ht_get(t, "Number"),
