...
void teardown(void) { rbt_free(&root); }
...
Test(treetests, is_valid_correct_tree, .fini = teardown) {
  ...
  cr_expect(rbt_is_valid(root), "rbt_is_valid should return TRUE if tree is "
                                "valid red-black binary search tree");
}

Test(treetests, contains_existing, .fini = teardown) {
  insert_from_n_to_m(1, 10);

  cr_expect(rbt_contains(root, 3),
...
Test(treetests, insert_1000, .fini = teardown) {
  ...
  cr_expect(rbt_is_valid(root),
            "inserting 1000 elements should produce a valid tree");
  for (int i = 1; i < 1000; ++i) {
    cr_expect(rbt_contains(root, i),
